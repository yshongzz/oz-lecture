...
function removeBook(button) {
  ...
  if (bookIndex !== -1) {
    books.splice(bookIndex, 1); //bookIndex에서 시작하는 한개의 요소를 제거
  }
...
function toggleRental(button) {
  ...
  // const status = rental.getStatus(); // 객채 내의 메서드로 사용되어, 객채의 상태 값을 반환함
  const rental = rentals.find((rental) => {
    rental.getStatus().title === title;
  });
  if (!rental) return;
